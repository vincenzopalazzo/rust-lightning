...
///
/// By default, only the required fields are included (payer_signing_pubkey, payment_hash,
...
impl<'a> PayerProofBuilder<'a, ExplicitSigningKey> {
	...
	///
	/// `payer_note` is an optional note scoped to this proof that is committed to by the
	/// payer signature. It is independent of any [`InvoiceRequest::payer_note`] set during
	/// the payment flow.
	///
...
impl<'a, S: SigningStrategy> PayerProofBuilder<'a, S> {
	...
	/// types in the signature/payer-proof range (240..=1000), or types in the
	/// unsupported gap before experimental BOLT12 TLV ranges begin.
...
impl UnsignedPayerProof<'_> {
	...
	pub fn sign<F: SignPayerProofFn>(mut self, sign: F) -> Result<PayerProof, PayerProofError> {
...
impl PayerProof {
	...
	/// committed to by the [`payer_signature`] alongside the invoice's merkle root.
	///
	/// [`InvoiceRequest::payer_note`]: crate::offers::invoice_request::InvoiceRequest::payer_note
	...
	fn try_from(tlv_stream: FullPayerProofTlvStream) -> Result<Self, Self::Error> {
		...
		Ok(Self {
			...
			omitted_markers: omitted_markers
				...
				.map(|marker| marker.0)
				.collect(),
			missing_hashes: missing_hashes.unwrap_or_default(),
			leaf_hashes: leaf_hashes.unwrap_or_default(),
		})
	}
}
...
mod tests {
	...
	fn test_include_type_rejects_signature_types() {
		...
		assert!(matches!(
			check_include_type(EXPERIMENTAL_OFFER_TYPES.start - 1),
