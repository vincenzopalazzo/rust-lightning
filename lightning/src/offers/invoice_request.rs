...
pub struct InvoiceRequestBuilder<'a, 'b, T: secp256k1::Signing> {
	...
	bip_353_signing_key: Option<SecretKey>,
}
...
macro_rules! invoice_request_builder_methods { (
	...
	/// Building will fail with [`Bolt12SemanticError::InvalidPayerContact`] unless a return
	/// path is also set via [`Self::payer_offer`] or [`Self::payer_bip_353_name`].
	///
	/// Successive calls to this method will override the previous setting.
	///
	/// [`ContactSecrets`]: crate::offers::contacts::ContactSecrets
	/// [`Bolt12SemanticError::InvalidPayerContact`]: crate::offers::parse::Bolt12SemanticError::InvalidPayerContact
	...
	/// Sets the payer's own offer for BLIP-42 contact management.
	///
	/// This will include the serialized offer in the invoice request, allowing the recipient to
	/// pay us back and thereby establish a mutual contact relationship.
	///
	/// The offer's encoding must not exceed [`PAYER_OFFER_MAX_BYTES`], otherwise building the
	/// invoice request will fail with [`Bolt12SemanticError::InvalidPayerOffer`] unless a BIP
	/// 353 name is also set via [`Self::payer_bip_353_name`], in which case the oversized offer
	/// is omitted and the name is sent instead.
	///
	/// Successive calls to this method will override the previous setting.
	///
	...
	/// Reveals the payer's BIP 353 name and signs the invoice request with `offer_signing_key`.
	///
	/// `offer_signing_key` must be the private key of the offer published at `name` (the
	/// offer's `offer_issuer_id`, or the last hop of its first path if it has no issuer id).
	/// The signature is computed at build time over the invoice request merkle root with tag
	/// `lightning` || `invoice_request` || `invreq_payer_bip_353_signature`.
	///
	/// Per BLIP 42, a contact secret must also be set via [`Self::contact_secrets`].
	///
	/// Successive calls to this method will override the previous setting.
	pub fn payer_bip_353_name(
		$($self_mut)* $self: $self_type, name: HumanReadableName, offer_signing_key: SecretKey,
	) -> $return_type {
		$self.invoice_request.invreq_payer_bip_353_name = Some(name);
		$self.bip_353_signing_key = Some(offer_signing_key);
		$return_value
	}
	...
	> {
		...
		if let Some(payer_offer) = &$self.invoice_request.invreq_payer_offer {
			if payer_offer.as_ref().len() > PAYER_OFFER_MAX_BYTES {
				if $self.invoice_request.invreq_payer_bip_353_name.is_some() {
					// BLIP 42: SHOULD NOT include an offer over 300 bytes; send the BIP 353
					// name instead.
					$self.invoice_request.invreq_payer_offer = None;
				} else {
		...
		let has_secret = $self.invoice_request.invreq_contact_secret.is_some();
		let has_return_path = $self.invoice_request.invreq_payer_offer.is_some()
			|| $self.invoice_request.invreq_payer_bip_353_name.is_some();
		if has_secret != has_return_path {
			return Err(Bolt12SemanticError::InvalidPayerContact);
		}
		if $self.invoice_request.invreq_payer_bip_353_name.is_some()
			&& $self.bip_353_signing_key.is_none()
		{
		...
		(UnsignedInvoiceRequest, Option<Keypair>, Option<&'b Secp256k1<$secp_context>>)
	{
		...
		let mut unsigned_invoice_request = UnsignedInvoiceRequest::new($self.offer, invoice_request);
		if let Some(signing_key) = $self.bip_353_signing_key {
			unsigned_invoice_request.add_payer_bip_353_signature(&signing_key, secp_ctx.unwrap());
		}
	...
	for InvoiceRequestBuilder<'a, 'b, secp256k1::All>
{
	fn from(builder: InvoiceRequestWithDerivedPayerSigningPubkeyBuilder<'a, 'b>) -> Self {
		...
		Self { offer, invoice_request, payer_signing_pubkey, secp_ctx, bip_353_signing_key }
	}
}
...
impl UnsignedInvoiceRequest {
	fn new(offer: &Offer, contents: InvoiceRequestContents) -> Self {
		...
		// Allocate sufficient capacity for the common case of the experimental TLV fields:
		// invreq_contact_secret (~40 bytes) plus a compact invreq_payer_offer (~210 bytes).
		// Underestimating merely results in a reallocation.
		const EXPERIMENTAL_TLV_ALLOCATION_SIZE: usize = 250;
	...
	/// Signs `invreq_payer_bip_353_signature` over the invoice request and appends it to the
	/// experimental TLV stream. Recomputes the invoice-request tagged hash so the payer
	/// signature covers the BIP 353 signature record.
	fn add_payer_bip_353_signature<T: secp256k1::Signing>(
		&mut self, signing_key: &SecretKey, secp_ctx: &Secp256k1<T>,
	) {
		let tlv_stream =
			TlvStream::new(&self.bytes).chain(TlvStream::new(&self.experimental_bytes));
		let message = TaggedHash::from_tlv_stream(PAYER_BIP_353_SIGNATURE_TAG, tlv_stream);
		let keys = Keypair::from_secret_key(secp_ctx, signing_key);
		let signature = secp_ctx.sign_schnorr_no_aux_rand(message.as_digest(), &keys);
		self.contents.inner.invreq_payer_bip_353_signature =
			Some(PayerBip353Signature { offer_signing_key: keys.public_key(), signature });

		let offer_exp_end = TlvStream::new(&self.experimental_bytes)
			.range(EXPERIMENTAL_OFFER_TYPES)
			.last()
			.map_or(0, |last_record| last_record.end);
		self.experimental_bytes.truncate(offer_exp_end);
		let (_, _, _, _, experimental_invoice_request) = self.contents.inner.as_tlv_stream();
		experimental_invoice_request.write(&mut self.experimental_bytes).unwrap();
...
macro_rules! invoice_request_accessors { ($self: ident, $contents: expr) => {
	...
	/// Returns the payer's own offer if present in the invoice request, allowing us to pay them
...
impl Readable for PayerFields {
	fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		// parsed, so failing to parse them here indicates the stored data was corrupted.
		let payer_offer = payer_offer_bytes
			.map(Offer::try_from)
			.transpose()
...
mod tests {
	...
	#[test]
	fn fails_building_invoice_request_with_contact_secret_without_return_path() {
		let expanded_key = ExpandedKey::new([42; 32]);
		let entropy = FixedEntropy {};
		let nonce = Nonce::from_entropy_source(&entropy);
		let secp_ctx = Secp256k1::new();
	...
	#[test]
	fn fails_building_invoice_request_with_payer_offer_without_contact_secret() {
		let expanded_key = ExpandedKey::new([42; 32]);
		let entropy = FixedEntropy {};
		let nonce = Nonce::from_entropy_source(&entropy);
		let secp_ctx = Secp256k1::new();
	...
	#[test]
	fn builds_invoice_request_with_bip_353_name() {
		use crate::onion_message::dns_resolution::HumanReadableName;
		...
		let offer_key = recipient_keys().secret_key();
		...
		let invoice_request = OfferBuilder::new(recipient_pubkey())
			...
			.unwrap()
			.contact_secrets(contact_secrets.clone())
			.payer_bip_353_name(name.clone(), offer_key)
			.build_and_sign()
			.unwrap();
		...
		assert_eq!(payer_bip_353_name.name, name);
		assert_eq!(payer_bip_353_name.offer_signing_key, recipient_pubkey());

		let mut buffer = Vec::new();
		invoice_request.write(&mut buffer).unwrap();
		let parsed = InvoiceRequest::try_from(buffer).unwrap();
		assert_eq!(parsed.payer_bip_353_name(), invoice_request.payer_bip_353_name());

		let published_offer =
			OfferBuilder::new(recipient_pubkey()).amount_msats(1).build().unwrap();
		assert!(parsed.payer_bip_353_name().unwrap().matches_offer(&published_offer).is_ok());
	...
	#[test]
	fn omits_oversized_payer_offer_when_bip_353_name_is_set() {
		use crate::onion_message::dns_resolution::HumanReadableName;
		...
		let invoice_request = OfferBuilder::new(recipient_pubkey())
			...
			.payer_bip_353_name(name, recipient_keys().secret_key())
			.build_and_sign()
			.unwrap();
		assert_eq!(invoice_request.payer_offer(), None);
		assert!(invoice_request.payer_bip_353_name().is_some());
	}
	...
	fn invoice_request_fields_round_trip_with_bip_353_name() {
		...
		let deserialized = PayerFields::read(&mut io::Cursor::new(&buffer)).unwrap();
	...
	fn applies_known_contact_rule_to_invoice_request_fields() {
		...
		let anonymous = PayerFields { contact_secret: None, ..fields };
