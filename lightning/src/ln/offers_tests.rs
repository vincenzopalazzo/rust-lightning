...
use crate::offers::contacts::{compute_contact_secret, ContactSecret, ContactSecrets};
...
fn pay_offer_and_add_contacts_info_blip42() {
	...
	assert!(invoice_request.contact_secret().is_some());
	assert_eq!(invoice_request.contact_secret().unwrap(), *contact_info.contact_secrets.primary_secret().as_bytes());
	...
	assert_eq!(alice_invoice_request_fields.contact_secret.unwrap(), *contact_info.contact_secrets.primary_secret().as_bytes());
	...
	alice.node.pay_for_offer(&alice_payer_offer, Some(5_000_000), payment_id, OptionalOfferPaymentParams{
		contact_secrects: Some(ContactSecrets::new(ContactSecret::new(alice_contact_secret))),
