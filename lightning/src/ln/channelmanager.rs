...
use crate::events::{
	...
	MessageSendEvent, MessageSendEventsProvider, PaymentFailureReason, ReplayEvent,
};
...
use crate::ln::channel::{
	self, Channel, ChannelError, ChannelPhase, ChannelUpdateStatus, InboundV1Channel,
	InteractivelyFunded as _, OutboundV1Channel, ShutdownResult, UpdateFulfillCommitFetch,
	WithChannelContext,
};
	...
	($self: ident, $peer_state: expr, $res: expr, $entry: expr) => {
		match $res {
			...
			Err(e) => {
				let key = *$entry.key();
				let (drop, res) =
					convert_chan_phase_err!($self, $peer_state, e, $entry.get_mut(), &key);
				if drop {
					$entry.remove_entry();
				}
				break Err(res);
	...
	L::Target: Logger,
{
				...
				hash_map::Entry::Occupied(mut chan_phase_entry) => {
					if let ChannelPhase::Funded(chan) = chan_phase_entry.get_mut() {
						...
						let mut shutdown_res =
							chan_phase_entry.get_mut().context_mut().force_shutdown(
								false,
					...
					| ChannelPhase::UnfundedInboundV2(_) => {
						...
						(
							chan_phase_entry
								.get_mut()
								.context_mut()
								.force_shutdown(false, closure_reason),
							None,
						)
					},
				};
	...
	fn htlc_failure_from_update_add_err(
		...
		err_code: u16, is_intro_node_blinded_forward: bool, shared_secret: &[u8; 32],
	) -> HTLCFailureMsg {
				...
				for (channel_id, counterparty_node_id) in channels_to_remove {
					per_peer_state
						...
						.map(|(mut chan, mut peer_state)| {
							...
							let mut close_res =
								chan.context_mut().force_shutdown(false, closure_reason);
					...
					peer_state.channel_by_id.retain(|chan_id, phase| match phase {
						ChannelPhase::Funded(chan) => {
							...
							if let Err(e) = chan.timer_check_closing_negotiation_progress() {
								let (needs_close, err) = convert_chan_phase_err!(
									self,
									peer_state,
									e,
									chan,
...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
	...
	fn do_accept_inbound_channel(
		...
		_funding_inputs: Vec<(TxIn, TransactionU16LenLimited)>, _total_witness_weight: Weight,
	) -> Result<(), APIError> {
		let logger = WithContext::from(
			&self.logger,
			Some(*counterparty_node_id),
		...
		let res = match peer_state.inbound_channel_request_by_id.remove(temporary_channel_id) {
			Some(unaccepted_channel) => {
				...
				match unaccepted_channel.open_channel_msg {
					...
					#[cfg(dual_funding)]
					OpenChannelMessage::V2(open_channel_msg) => InboundV2Channel::new(
						&self.fee_estimator,
						&self.entropy_source,
						&self.signer_provider,
	...
	fn internal_open_channel(
		...
		let (mut channel_phase, message_send_event) = match msg {
			...
			OpenChannelMessageRef::V2(msg) => {
				let channel = InboundV2Channel::new(
					&self.fee_estimator,
					&self.entropy_source,
					&self.signer_provider,
					self.get_our_node_id(),
	...
	fn internal_tx_msg<
		HandleTxMsgFn: Fn(&mut ChannelPhase<SP>) -> Result<MessageSendEvent, &'static str>,
	>(
	...
	fn internal_tx_add_input(
		...
		self.internal_tx_msg(
			&counterparty_node_id,
			msg.channel_id,
			|channel_phase: &mut ChannelPhase<SP>| match channel_phase {
				...
				peer_state.channel_by_id.entry(msg.channel_id.clone())
			{
				...
				match phase {
					...
					| ChannelPhase::UnfundedOutboundV2(_) => {
						...
						let mut close_res = phase.context_mut().force_shutdown(
							false,
						...
						if let Some(counterparty_node_id) = counterparty_node_id_opt {
								...
								per_peer_state.get(&counterparty_node_id)
							{
									...
									peer_state.channel_by_id.entry(channel_id)
								{
									...
									let mut shutdown_res = chan_phase_entry
										.get_mut()
										.context_mut()
										.force_shutdown(false, reason.clone());
									let chan_phase = remove_channel_phase!(
										self,
										peer_state,
										chan_phase_entry,
										shutdown_res
			...
			match phase {
				...
				ChannelPhase::UnfundedOutboundV1(chan) => {
					let (open_channel, funding_created) =
						chan.signer_maybe_unblocked(self.chain_hash.clone(), &self.logger);
					if let Some(msg) = open_channel {
						pending_msg_events
							.push(events::MessageSendEvent::SendOpenChannel { node_id, msg });
					}
				...
				ChannelPhase::UnfundedInboundV1(chan) => {
					...
					if let Some(msg) = chan.signer_maybe_unblocked(&&logger) {
						pending_msg_events
							.push(events::MessageSendEvent::SendAcceptChannel { node_id, msg });
					}
					None
				},
...
impl<
		...
		MR: Deref,
		L: Deref,
	> MessageSendEventsProvider for ChannelManager<M, T, ES, NS, SP, F, R, MR, L>
where
	M::Target: chain::Watch<<SP::Target as SignerProvider>::EcdsaSigner>,
	T::Target: BroadcasterInterface,
	ES::Target: EntropySource,
		...
		} else {
			{
				...
				match peer_state.channel_by_id.get_mut(&msg.channel_id) {
					Some(ChannelPhase::UnfundedOutboundV1(ref mut chan)) => {
						let logger = WithChannelContext::from(&self.logger, &chan.context, None);
						if let Ok(msg) = chan.maybe_handle_error_without_close(
							self.chain_hash,
							&self.fee_estimator,
							&&logger,
						) {
		...
		match message {
			OffersMessage::InvoiceRequest(invoice_request) => {
				...
				if self.default_configuration.notify_bolt12_invoice_sent {
					if let Ok(ref invoice) = response {
	...
	L::Target: Logger,
{
	...
	pub channel_monitors:
		HashMap<OutPoint, &'a ChannelMonitor<<SP::Target as SignerProvider>::EcdsaSigner>>,
}
	...
	fn read<Reader: io::Read>(
			...
			if let Some(ref mut monitor) = args.channel_monitors.get_mut(&funding_txo) {
						...
						< monitor.get_latest_update_id()
				{
					...
					per_peer_state
						...
						.unwrap()
						.channel_by_id
						.insert(channel.context.channel_id(), ChannelPhase::Funded(channel));
				}
		...
		'each_bg_event: for mut new_event in close_background_events {
			...
			} = &mut new_event
			{
				...
				let in_flight_updates = per_peer_state
					.in_flight_monitor_updates
					.entry(*funding_txo)
					.or_insert_with(Vec::new);
				...
				if !payment_claims.is_empty() {
					...
					if let Some(payment) = payment {
						...
						for claimable_htlc in &payment.htlcs {
							...
							let peer_node_id_opt = channel_manager
								.outpoint_to_peer
								.lock()
								.unwrap()
								.get(&claimable_htlc.prev_hop.outpoint)
								.cloned();
