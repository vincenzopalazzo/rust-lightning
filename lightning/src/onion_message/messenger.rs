...
pub trait AOnionMessenger {
	...
	type PNH: PosNotificationHandler;
...
pub enum PeeledOnion<T: OnionMessageContents> {
	...
	PosNotification(
		PosNotificationMessage,
		Option<PosNotificationContext>,
			...
			Ok(PeeledOnion::PosNotification(message, _context, reply_path)) => {
				...
				match message {
					PosNotificationMessage::PaymentNotification(msg) => {
						let response_instructions = self
							.pos_notification_handler
							.handle_payment_notification(msg, responder);
						if let Some((msg, instructions)) = response_instructions {
							let _ = self.handle_onion_message_response(msg, instructions);
						}
					},
					PosNotificationMessage::PaymentAck(msg) => {
						self.pos_notification_handler.handle_payment_ack(msg);
					},
					PosNotificationMessage::PaymentNack(msg) => {
						self.pos_notification_handler.handle_payment_nack(msg);
					},
				}
			},
